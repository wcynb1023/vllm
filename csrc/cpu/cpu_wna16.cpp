...
class Dequantizer4b {
 ...
 private:
  ...
  static vec_op::FP32Vec16 load_4b_vec(uint64_t q_values) {
#if defined(__riscv_v_min_vlen) && __riscv_v_min_vlen == 128
    if constexpr (isa == ISA::RVV) {
      return load_4b_vec_rvv(q_values);
    }
#endif
    alignas(64) float values[16];
    for (int32_t i = 0; i < 16; ++i) {
      int32_t q_value = (q_values >> (i * 4)) & 0xF;
      if constexpr (!has_zp) {
        q_value -= 8;
      }
      values[i] = static_cast<float>(q_value);
    }
 ...
 public:
  static void dequant(int32_t* __restrict__ q_weight,
                      ...
                      const int64_t scales_stride, const int64_t zeros_stride,
                      const int32_t k_size, const int32_t group_size) {
    uint64_t* __restrict__ curr_q_weight =
        reinterpret_cast<uint64_t*>(q_weight);
    uint64_t* __restrict__ curr_zeros = reinterpret_cast<uint64_t*>(zeros);
    scalar_t* __restrict__ curr_weight = weight;
    scalar_t* __restrict__ curr_scale = scales;
    vec_op::FP32Vec16 scale_0;
    vec_op::FP32Vec16 scale_1;
    ...
    int32_t group_counter = 0;
    for (int32_t k_idx = 0; k_idx < k_size; k_idx += 2) {
      vec_op::FP32Vec16 wb_0 = load_4b_vec(*curr_q_weight);
      vec_op::FP32Vec16 wb_1 = load_4b_vec(*(curr_q_weight + 1));
      ...
      if constexpr (!use_desc_act) {
        ...
        scale_1 = vec_op::FP32Vec16(
            scalar_vec_t(curr_scale + g_idx_1 * scales_stride));
        if constexpr (has_zp) {
          zero_0 = load_4b_vec(*(curr_zeros + g_idx_0 * zeros_stride / 2));
          zero_1 = load_4b_vec(*(curr_zeros + g_idx_1 * zeros_stride / 2));
        }
      }
